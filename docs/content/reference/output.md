...
describes what users get. Pick a format with `-o`, or let brilliant choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
brilliant <command> -o table   # aligned columns for reading
brilliant <command> -o jsonl   # one JSON object per line, for piping
brilliant <command> -o json    # a single JSON array
brilliant <command> -o csv     # spreadsheet friendly
brilliant <command> -o tsv     # tab-separated
brilliant <command> -o url     # just the URL column
brilliant <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
brilliant <command> --fields id,title,url
```
...
```bash
brilliant <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
brilliant <command>            # a table, because this is a terminal
brilliant <command> | wc -l    # JSONL, because this is a pipe
```
